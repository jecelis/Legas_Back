...
userRuta.post("/consultar", function (req, res) {
  ...
  nominaModelo.findOne({ cedula }, function (error, emp) {
    if (error) {
      return res
        .status(401)
        .send({ estado: "Error", msg: "Empleado no encontrado" });
    } else {
      if (emp !== null) {
        return res
          .status(200)
          .send({ estado: "Ok", msg: "Empleado encontrado", dato: emp });
      } else {
    ...
    nominaModelo.updateOne(
      ...
      function (error) {
        if (error) {
          return res
            .status(401)
            .send({ estado: "Error", msg: "Empleado no actualizado" });
        }
        return res
          .status(200)
          .send({ estado: "Ok", msg: "Empleado actualizado" });
      }
    );
  }
});
    ...
    nominaModelo.deleteOne(
      ...
      function (error) {
        if (error) {
          return res
            .status(401)
            .send({ estado: "Error", msg: "Empleado no eliminado" });
        }
        return res
          .status(200)
          .send({ estado: "Ok", msg: "Empleado eliminado" });
      }
    );
  }
});

userRuta.get("/listar", function (req,res){

  nominaModelo.find({}, function(error,jug){
      if(error){
          return res.status(401).send({estado : "Error", msg : "Jugadores no encontrados"});
      }
      else{
          if(jug!==null){
              return res.status(200).send({estado : "Ok", msg : "Jugadores encontrados", dato:jug});
              console.log(jug) 
          }
          else{
              return res.send({estado : "Error", msg : "Jugadores NO encontrados"});
          }
      }
